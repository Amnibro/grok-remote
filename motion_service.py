...
IDLE_DWELL = {"standing_w_briefcase_idle": 59.0, "talking_on_phone": 59.0, "guitar_playing": 59.0}
...
async def alive_loop(app):
    nxt = random.uniform(4, 112)
    since = 0.0
    while True:
        now = time.time()
        nap = random.uniform(4, 112)
        ...
        did = False
        life_clip = None
        if quiet >= IDLE_DWELL.get(state.get("base"), 59.0):
            win = REPEAT_WINDOW
            def ok(c, rec=True):
