...
IDLE_DWELL = {"standing_w_briefcase_idle": 266.0, "talking_on_phone": 266.0, "guitar_playing": 266.0}
...
async def alive_loop(app):
    nxt = random.uniform(4, 528)
    since = 0.0
    while True:
        now = time.time()
        nap = random.uniform(4, 528)
        ...
        did = False
        life_clip = None
        if quiet >= IDLE_DWELL.get(state.get("base"), 266.0):
            win = REPEAT_WINDOW
            def ok(c, rec=True):
