...
IDLE_DWELL = {"standing_w_briefcase_idle": 68.0, "talking_on_phone": 68.0, "guitar_playing": 68.0}
...
async def alive_loop(app):
    nxt = random.uniform(4, 132)
    since = 0.0
    while True:
        now = time.time()
        nap = random.uniform(4, 132)
        ...
        did = False
        life_clip = None
        if quiet >= IDLE_DWELL.get(state.get("base"), 68.0):
            win = REPEAT_WINDOW
            def ok(c, rec=True):
