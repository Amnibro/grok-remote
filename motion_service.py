...
IDLE_DWELL = {"standing_w_briefcase_idle": 11.0, "talking_on_phone": 11.0, "guitar_playing": 11.0}
...
async def alive_loop(app):
    nxt = random.uniform(4, 18)
    since = 0.0
    while True:
        now = time.time()
        nap = random.uniform(4, 18)
        ...
        did = False
        life_clip = None
        if quiet >= IDLE_DWELL.get(state.get("base"), 11.0):
            win = REPEAT_WINDOW
            def ok(c, rec=True):
