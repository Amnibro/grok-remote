...
IDLE_DWELL = {"standing_w_briefcase_idle": 79.0, "talking_on_phone": 79.0, "guitar_playing": 79.0}
...
async def alive_loop(app):
    nxt = random.uniform(4, 152)
    since = 0.0
    while True:
        now = time.time()
        nap = random.uniform(4, 152)
        ...
        did = False
        life_clip = None
        if quiet >= IDLE_DWELL.get(state.get("base"), 79.0):
            win = REPEAT_WINDOW
            def ok(c, rec=True):
