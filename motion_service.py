...
IDLE_DWELL = {"standing_w_briefcase_idle": 269.0, "talking_on_phone": 269.0, "guitar_playing": 269.0}
...
async def alive_loop(app):
    nxt = random.uniform(4, 532)
    since = 0.0
    while True:
        now = time.time()
        nap = random.uniform(4, 532)
        ...
        did = False
        life_clip = None
        if quiet >= IDLE_DWELL.get(state.get("base"), 269.0):
            win = REPEAT_WINDOW
            def ok(c, rec=True):
