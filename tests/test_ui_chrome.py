...
def test_idle_chain_uses_seamless_mixamo():
    ...
    assert "uniform(4, 52)" in alive[alive.find('"idle recover"'):alive.find("quiet =")]
    ...
    assert "uniform(4, 52)" in alive[alive.find("if dwell < need"):alive.find("pick = pick_chain")]
    ...
    assert "uniform(4, 52)" in alive[alive.find('"idle chain"'):alive.find("if busy")]
    assert "nxt = random.uniform(4, 52)" in alive[:alive.find("while True")]
    assert "nxt = random.uniform(4, 52)" in alive[alive.find("if not clients"):alive.find("since += ")]
    assert "nap = random.uniform(4, 52)" in alive[alive.find("while True"):alive.find("if state.get(\"follow_base\")")]
    ...
    assert '"standing_w_briefcase_idle": 28.0' in srv[srv.find("IDLE_DWELL"):srv.find("LIFE =")]
    assert '"talking_on_phone": 28.0' in srv[srv.find("IDLE_DWELL"):srv.find("LIFE =")]
    assert '"guitar_playing": 28.0' in srv[srv.find("IDLE_DWELL"):srv.find("LIFE =")]
    ...
    assert "rephase_at\", 0) >= 4" not in srv and "rephase_at', 0)>=4" not in srv.replace(" ","")
    assert "uniform(4, 52)" in srv[srv.find("rephase_at"):srv.find("idle rephase")]
