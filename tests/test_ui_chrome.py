...
def test_idle_chain_uses_seamless_mixamo():
    ...
    assert "random.random() < 0.20" in pc
    ...
    assert "win = 8.0 if len(pool) <= 2 else 10.0" in srv
    ...
    assert "2 if clip_dur(c) <= 2.2 else 1" in srv
    ...
    assert "rephase_at\", 0) < 4" in srv or "rephase_at', 0)<4" in srv.replace(" ","")
    assert "rephase_at\", 0) >= 4" in srv or "rephase_at', 0)>=4" in srv.replace(" ","")
