...
# Track ONLY our PIDs — never Stop-Process -Name grok
$script:OurPids = @()
function Stop-Ours {
...
# Free UI/agent ports if WE left leftovers (by port owner only when safe)
...
$agentBat = Join-Path $logDir "run-agent.cmd"
# Agent stays on loopback — phone never hits it directly
@"
@echo off
cd /d `"$Cwd`"
set GROK_AGENT_SECRET=$Secret
REM --leader shares live state with the desktop TUI when a leader is running
`"$Grok`" agent --always-approve --leader serve --bind 127.0.0.1:$Port --secret $Secret >> `"$logDir\agent.log`" 2>&1
...
Write-Host ""
$agent = Start-Process -FilePath "cmd.exe" -ArgumentList @("/c", $agentBat) -PassThru -WindowStyle Hidden
...
if (-not $NoUi) {
  ...
  # Install aiohttp quietly if needed
  ...
  if (-not $uiOk) {
    Write-Host "UI failed to bind 0.0.0.0:$UiPort — is an old server on 127.0.0.1 only?" -ForegroundColor Red
