...
## 2026-07-12 â€” Chat switch history: one bubble per turn (v25)

- **Switch-only bug:** disk history is coalesced full turns, but the live streaming buffer still reused one You/Grok bubble â€” older turns vanished when opening another chat
...
## 2026-07-12 Ã¢â‚¬â€ History: restore You bubbles on refresh (v24)

- **Bug:** chat-only disk history has no `turn_completed`, so `curUser` stayed open and every user line overwrote the first You bubble Ã¢â‚¬â€ looked like older messages vanished
...
## 2026-07-12 Ã¢â‚¬â€ Composer shake + hide agent setup in chat (v23)
...
## 2026-07-12 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Command deck menu + todo badge fix (v22)

- **Why ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œTodo 4ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â:** `paintTodoBadge` overwrote the whole Tools row button with `todo N` (open count). Now keeps **Todos** label + badge key (`3/4`, `board`)
- **Command deck:** glass panel, brand header, live status chips, icon rows, pop-in, 3ÃƒÆ’Ã¢â‚¬â€3 glyph trigger (replaces plain ÃƒÂ¢Ã‹Å“Ã‚Â°)
- Tools inject with icons; status bar refreshes while menu open

## 2026-07-12 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ÃƒÂ¢Ã‹Å“Ã‚Â° menu dedupe (v21)

- Flattened upper-right menu: Chat Ãƒâ€šÃ‚Â· Tools Ãƒâ€šÃ‚Â· Hide Ãƒâ€šÃ‚Â· Collapse Ãƒâ€šÃ‚Â· Apps Ãƒâ€šÃ‚Â· Appearance Ãƒâ€šÃ‚Â· Help
- **Permission** is one cycle control (Always ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ Ask ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ Plan), same as livebar
...
## 2026-07-12 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Keyboard clearance + rotate formatting (v20)
...
- **Rotate:** hard-reset body pin + CSS vars (`--vv-h`, `--feed-pad-bottom`, ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦); multi-stage reflow through ~1.6s; detect width/height flip as orientation
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Mobile keyboard + orientation reflow
...
- **Rotate** portraitÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Âlandscape: multi-stage reflow (reset composer height, layout, stack pad, scroll)
- Permission chip/menu; always-approve skips plan approve UI
- Markdown lettered lists + safer path chips

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Fix mixed history across sessions
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Start server reliability + faster chat open
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Fast history: last 100 + scroll-up load

- Open session loads only the **latest ~100** disk events (smaller read window)
- **Scroll up** (or tap ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œearlier messagesÃƒÂ¢Ã¢â€šÂ¬Ã‚Â) loads the previous page and prepends it
- API: `/api/session/history?limit=100&before=<byte>` + `meta.has_more` / `older_before`

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Footer de-clutter + menu fonts + Amni-Delve

- ÃƒÂ¢Ã‹Å“Ã‚Â° menu uses UI `--font` / size (no fixed 12px mono)
- Removed above-composer clutter: no more ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œnot a git repoÃƒÂ¢Ã¢â€šÂ¬Ã‚Â, cost strip, or always-on When busy row
- Git + turns/ctx move into the **livebar** (only when useful); Interject/Queue/FYI only while busy
- **Amni-Delve** always in ÃƒÂ¢Ã‹Å“Ã‚Â° Ãƒâ€šÃ‚Â· Voice & wear; owner auto-unlock for Anthony/Amni paths

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Fix new chat inheriting old user messages
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Composer UX + XR detect + Watch v19

- **+** is attach-only; **Mic** sits beside the composer for Go voice
- Special functions moved to top-right **ÃƒÂ¢Ã‹Å“Ã‚Â°** (Todos, Terminal, Git, Export, ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦)
- **Chat view** toggles in ÃƒÂ¢Ã‹Å“Ã‚Â°: hide/collapse thinking, tools, edits, reads, code
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Message queue modes + Cancel v18

- **Interject / Queue / FYI** chips above composer (`When busy`)
- **Interject:** cancel current steps ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ send now
- **Queue:** hold until a good pause (`turn_completed`), then send with guidance framing
- **FYI:** extra context only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never cancels; applied at next pause with ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œdonÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t stopÃƒÂ¢Ã¢â€šÂ¬Ã‚Â framing
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Voice Go + XR/AR + Grok TTS v17
...
- **Real Grok voice:** `POST /api/tts` ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `https://api.x.ai/v1/tts` (needs `XAI_API_KEY`); browser `speechSynthesis` fallback
- `GET /api/voice/status` reports TTS readiness; voice picker (eve, ara, leo, ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦)

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Promote archived ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ live on message v16

- Messaging an **archived** session unarchives it (`POST {id, archived:false}`), marks it **live**/resident, switches scope Archived ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ Active, and **auto-refreshes** the session list
...
## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Polish v15 (syntax, greyscale, tools, tour, repo)
...
## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Spoiler-safe Grok screenshots v14
...
- README shots retaken ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no personal paths/chats, Grok dark default, phone without Alt

## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Menu stacking + code line numbers / syntax

- Header ÃƒÂ¢Ã‹Å“Ã‚Â° / orbit menus use `position:fixed` + high z-index so they paint above chat
...
## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Live CLIÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢remote disk catch-up
...
## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Start Server + port claim + reconnect guard v8

- **Start Server** header + setup button ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `POST /api/stack/start` (kills stale :2419, spawns `grok agent --no-leader serve`, rebinds hub)
- **Boot:** UI claims free :2421 listeners; auto-starts agent if missing; `start.ps1` kills :2419/:2421 before bind
- **Reconnect:** max 5 tries, no infinite reconnecting #N loop; agent error no longer thrashing; after give-up ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ hit Start Server
- Auto-connect: if `/health` agent down, Start Server first then connect

## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Live status + init/session reliability v7
...
## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Pin fix + feature tour v6

- **Pin bugfix:** local `import subprocess` inside `main_async` except-block made nested handlers fail with *cannot access free variable 'subprocess'ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦* when aiohttp was already installed. Use module-level `subprocess` only.
...
## 2026-07-10 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Stop button + pin/start from anywhere

- UI header **Stop** ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `POST /api/stack/stop` (UI + agent serve only; never mass-kill grok)
- UI **Pin** ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `POST /api/stack/shortcut` creates Desktop + Start Menu shortcuts
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Claude-gap pack v5.1

Closer to Claude Code day-to-day flow:

- **Stop turn** button (session/cancel) while agent is busy
- **Git strip** branch/sha/dirty + `/api/git/status|diff|log` + ÃƒÅ½Ã¢â‚¬Â working-tree diff
...
- **Open tool paths in IDE** (click ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Å¾ locs)
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â UX fix: contrast, horizon loader, terminal lag v4.2

- Contrast: solid bubbles, readable `--tx`/`--soft`/`--mut`, softened Grok mono (no pure black/white)
- **Event horizon loader** lives in `#chatStage` (chat area above composer only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not header/fullscreen)
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â xAI product UX polish v4.1b
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Claude-gap cockpit features v5.0
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Full skills palette v4.3
...
- Advanced skills (design, execute-plan, review, ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦) appear even if agent list is sparse
- Restart UI server required

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Multi-client hub + history snap v4.2
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Personas & directions v4.1

- **Persona flyout:** personalities (Concise, Unhinged, Programmer, Engineer, Manager, Clown, Warlord, ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦)
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Cockpit IDE + auto-stack + Grok Review v4.0

- **Electron auto-stack:** launch app ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ agent + UI proxy (no daily terminal)
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â X-like labels + Ubuntu Yaru-ish v3.7
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Tool payloads + rename skins v3.6
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Code colors + collapsible rail + flyouts v3.5

- **Syntax colors** in fenced code: keywords/strings/numbers/comments/fns use theme tokens (`--acc`, `--ok`, `--you`, ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦)
- **Collapsible sessions rail:** Ãƒâ€šÃ‚Â« Hide / ÃƒÂ¢Ã‹Å“Ã‚Â° / edge tab; pref `grok_remote_sidebar`
- **Top-right flyouts:** Theme, Skills, Task open frosted glass panels sliding from top-right (Esc closes)

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Legacy themes v3.4
...
- Theme picker groups **Product** then **Legacy Ãƒâ€šÃ‚Â· retro**
- Win95 bevel chrome, Amiga/Win95 title bars, Matrix phosphor glow

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Chat polish + attachments v3.3
...
- **File upload:** ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â½ attach / drag-drop / paste; images as ACP `image` blocks; text/code as resource + fenced body
- Limits: 8 files, 6MB each Ãƒâ€šÃ‚Â· hard-refresh clients (`?v=3`)

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SpaceXAI beauty + session isolation v3.2
...
## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Sessions scroll + jump + archive v2

- **ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Å“ Bottom** always on in chat (not only when scrolled up); forced scroll + `scrollIntoView`
- **Independent scrolls:** body locked; session rail list vs chat feed scroll separately
- **Archive / Unarchive** per session (localStorage); scope chips Active Ãƒâ€šÃ‚Â· Live Ãƒâ€šÃ‚Â· Archived Ãƒâ€šÃ‚Â· All + search
- Layout: picker-head sticky controls, `.sess` flex scroll region

## 2026-07-09 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â UX options v1

- **Auto-scroll:** toggle on setup/Theme UX chips; stick-to-bottom while reading live; jump FAB
- **Collapse:** thinking rows, fenced code, tool cards ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â defaults + per-block tap
