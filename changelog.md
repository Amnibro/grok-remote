...
## 2026-07-12 â€” History: restore You bubbles on refresh (v24)

- **Bug:** chat-only disk history has no `turn_completed`, so `curUser` stayed open and every user line overwrote the first You bubble â€” looked like older messages vanished
...
## 2026-07-12 â€” Composer shake + hide agent setup in chat (v23)
...
## 2026-07-12 Ã¢â‚¬â€ Command deck menu + todo badge fix (v22)

- **Why Ã¢â‚¬Å“Todo 4Ã¢â‚¬Â:** `paintTodoBadge` overwrote the whole Tools row button with `todo N` (open count). Now keeps **Todos** label + badge key (`3/4`, `board`)
- **Command deck:** glass panel, brand header, live status chips, icon rows, pop-in, 3Ãƒâ€”3 glyph trigger (replaces plain Ã¢ËœÂ°)
- Tools inject with icons; status bar refreshes while menu open

## 2026-07-12 Ã¢â‚¬â€ Ã¢ËœÂ° menu dedupe (v21)

- Flattened upper-right menu: Chat Ã‚Â· Tools Ã‚Â· Hide Ã‚Â· Collapse Ã‚Â· Apps Ã‚Â· Appearance Ã‚Â· Help
- **Permission** is one cycle control (Always Ã¢â€ â€™ Ask Ã¢â€ â€™ Plan), same as livebar
...
## 2026-07-12 Ã¢â‚¬â€ Keyboard clearance + rotate formatting (v20)
...
- **Rotate:** hard-reset body pin + CSS vars (`--vv-h`, `--feed-pad-bottom`, Ã¢â‚¬Â¦); multi-stage reflow through ~1.6s; detect width/height flip as orientation
...
## 2026-07-09 Ã¢â‚¬â€ Mobile keyboard + orientation reflow
...
- **Rotate** portraitÃ¢â€ â€landscape: multi-stage reflow (reset composer height, layout, stack pad, scroll)
- Permission chip/menu; always-approve skips plan approve UI
- Markdown lettered lists + safer path chips

## 2026-07-09 Ã¢â‚¬â€ Fix mixed history across sessions
...
## 2026-07-09 Ã¢â‚¬â€ Start server reliability + faster chat open
...
## 2026-07-09 Ã¢â‚¬â€ Fast history: last 100 + scroll-up load

- Open session loads only the **latest ~100** disk events (smaller read window)
- **Scroll up** (or tap Ã¢â‚¬Å“earlier messagesÃ¢â‚¬Â) loads the previous page and prepends it
- API: `/api/session/history?limit=100&before=<byte>` + `meta.has_more` / `older_before`

## 2026-07-09 Ã¢â‚¬â€ Footer de-clutter + menu fonts + Amni-Delve

- Ã¢ËœÂ° menu uses UI `--font` / size (no fixed 12px mono)
- Removed above-composer clutter: no more Ã¢â‚¬Å“not a git repoÃ¢â‚¬Â, cost strip, or always-on When busy row
- Git + turns/ctx move into the **livebar** (only when useful); Interject/Queue/FYI only while busy
- **Amni-Delve** always in Ã¢ËœÂ° Ã‚Â· Voice & wear; owner auto-unlock for Anthony/Amni paths

## 2026-07-09 Ã¢â‚¬â€ Fix new chat inheriting old user messages
...
## 2026-07-09 Ã¢â‚¬â€ Composer UX + XR detect + Watch v19

- **+** is attach-only; **Mic** sits beside the composer for Go voice
- Special functions moved to top-right **Ã¢ËœÂ°** (Todos, Terminal, Git, Export, Ã¢â‚¬Â¦)
- **Chat view** toggles in Ã¢ËœÂ°: hide/collapse thinking, tools, edits, reads, code
...
## 2026-07-09 Ã¢â‚¬â€ Message queue modes + Cancel v18

- **Interject / Queue / FYI** chips above composer (`When busy`)
- **Interject:** cancel current steps Ã¢â€ â€™ send now
- **Queue:** hold until a good pause (`turn_completed`), then send with guidance framing
- **FYI:** extra context only Ã¢â‚¬â€ never cancels; applied at next pause with Ã¢â‚¬Å“donÃ¢â‚¬â„¢t stopÃ¢â‚¬Â framing
...
## 2026-07-09 Ã¢â‚¬â€ Voice Go + XR/AR + Grok TTS v17
...
- **Real Grok voice:** `POST /api/tts` Ã¢â€ â€™ `https://api.x.ai/v1/tts` (needs `XAI_API_KEY`); browser `speechSynthesis` fallback
- `GET /api/voice/status` reports TTS readiness; voice picker (eve, ara, leo, Ã¢â‚¬Â¦)

## 2026-07-09 Ã¢â‚¬â€ Promote archived Ã¢â€ â€™ live on message v16

- Messaging an **archived** session unarchives it (`POST {id, archived:false}`), marks it **live**/resident, switches scope Archived Ã¢â€ â€™ Active, and **auto-refreshes** the session list
...
## 2026-07-10 Ã¢â‚¬â€ Polish v15 (syntax, greyscale, tools, tour, repo)
...
## 2026-07-10 Ã¢â‚¬â€ Spoiler-safe Grok screenshots v14
...
- README shots retaken Ã¢â‚¬â€ no personal paths/chats, Grok dark default, phone without Alt

## 2026-07-10 Ã¢â‚¬â€ Menu stacking + code line numbers / syntax

- Header Ã¢ËœÂ° / orbit menus use `position:fixed` + high z-index so they paint above chat
...
## 2026-07-10 Ã¢â‚¬â€ Live CLIÃ¢â€ â€™remote disk catch-up
...
## 2026-07-10 Ã¢â‚¬â€ Start Server + port claim + reconnect guard v8

- **Start Server** header + setup button Ã¢â€ â€™ `POST /api/stack/start` (kills stale :2419, spawns `grok agent --no-leader serve`, rebinds hub)
- **Boot:** UI claims free :2421 listeners; auto-starts agent if missing; `start.ps1` kills :2419/:2421 before bind
- **Reconnect:** max 5 tries, no infinite reconnecting #N loop; agent error no longer thrashing; after give-up Ã¢â€ â€™ hit Start Server
- Auto-connect: if `/health` agent down, Start Server first then connect

## 2026-07-10 Ã¢â‚¬â€ Live status + init/session reliability v7
...
## 2026-07-10 Ã¢â‚¬â€ Pin fix + feature tour v6

- **Pin bugfix:** local `import subprocess` inside `main_async` except-block made nested handlers fail with *cannot access free variable 'subprocess'Ã¢â‚¬Â¦* when aiohttp was already installed. Use module-level `subprocess` only.
...
## 2026-07-10 Ã¢â‚¬â€ Stop button + pin/start from anywhere

- UI header **Stop** Ã¢â€ â€™ `POST /api/stack/stop` (UI + agent serve only; never mass-kill grok)
- UI **Pin** Ã¢â€ â€™ `POST /api/stack/shortcut` creates Desktop + Start Menu shortcuts
...
## 2026-07-09 Ã¢â‚¬â€ Claude-gap pack v5.1

Closer to Claude Code day-to-day flow:

- **Stop turn** button (session/cancel) while agent is busy
- **Git strip** branch/sha/dirty + `/api/git/status|diff|log` + ÃŽâ€ working-tree diff
...
- **Open tool paths in IDE** (click Ã°Å¸â€œâ€ž locs)
...
## 2026-07-09 Ã¢â‚¬â€ UX fix: contrast, horizon loader, terminal lag v4.2

- Contrast: solid bubbles, readable `--tx`/`--soft`/`--mut`, softened Grok mono (no pure black/white)
- **Event horizon loader** lives in `#chatStage` (chat area above composer only Ã¢â‚¬â€ not header/fullscreen)
...
## 2026-07-09 Ã¢â‚¬â€ xAI product UX polish v4.1b
...
## 2026-07-09 Ã¢â‚¬â€ Claude-gap cockpit features v5.0
...
## 2026-07-09 Ã¢â‚¬â€ Full skills palette v4.3
...
- Advanced skills (design, execute-plan, review, Ã¢â‚¬Â¦) appear even if agent list is sparse
- Restart UI server required

## 2026-07-09 Ã¢â‚¬â€ Multi-client hub + history snap v4.2
...
## 2026-07-09 Ã¢â‚¬â€ Personas & directions v4.1

- **Persona flyout:** personalities (Concise, Unhinged, Programmer, Engineer, Manager, Clown, Warlord, Ã¢â‚¬Â¦)
...
## 2026-07-09 Ã¢â‚¬â€ Cockpit IDE + auto-stack + Grok Review v4.0

- **Electron auto-stack:** launch app Ã¢â€ â€™ agent + UI proxy (no daily terminal)
...
## 2026-07-09 Ã¢â‚¬â€ X-like labels + Ubuntu Yaru-ish v3.7
...
## 2026-07-09 Ã¢â‚¬â€ Tool payloads + rename skins v3.6
...
## 2026-07-09 Ã¢â‚¬â€ Code colors + collapsible rail + flyouts v3.5

- **Syntax colors** in fenced code: keywords/strings/numbers/comments/fns use theme tokens (`--acc`, `--ok`, `--you`, Ã¢â‚¬Â¦)
- **Collapsible sessions rail:** Ã‚Â« Hide / Ã¢ËœÂ° / edge tab; pref `grok_remote_sidebar`
- **Top-right flyouts:** Theme, Skills, Task open frosted glass panels sliding from top-right (Esc closes)

## 2026-07-09 Ã¢â‚¬â€ Legacy themes v3.4
...
- Theme picker groups **Product** then **Legacy Ã‚Â· retro**
- Win95 bevel chrome, Amiga/Win95 title bars, Matrix phosphor glow

## 2026-07-09 Ã¢â‚¬â€ Chat polish + attachments v3.3
...
- **File upload:** Ã°Å¸â€œÅ½ attach / drag-drop / paste; images as ACP `image` blocks; text/code as resource + fenced body
- Limits: 8 files, 6MB each Ã‚Â· hard-refresh clients (`?v=3`)

## 2026-07-09 Ã¢â‚¬â€ SpaceXAI beauty + session isolation v3.2
...
## 2026-07-09 Ã¢â‚¬â€ Sessions scroll + jump + archive v2

- **Ã¢â€ â€œ Bottom** always on in chat (not only when scrolled up); forced scroll + `scrollIntoView`
- **Independent scrolls:** body locked; session rail list vs chat feed scroll separately
- **Archive / Unarchive** per session (localStorage); scope chips Active Ã‚Â· Live Ã‚Â· Archived Ã‚Â· All + search
- Layout: picker-head sticky controls, `.sess` flex scroll region

## 2026-07-09 Ã¢â‚¬â€ UX options v1

- **Auto-scroll:** toggle on setup/Theme UX chips; stick-to-bottom while reading live; jump FAB
- **Collapse:** thinking rows, fenced code, tool cards Ã¢â‚¬â€ defaults + per-block tap
