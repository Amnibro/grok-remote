...
## 2026-07-12 â€” Command deck menu + todo badge fix (v22)

- **Why â€œTodo 4â€:** `paintTodoBadge` overwrote the whole Tools row button with `todo N` (open count). Now keeps **Todos** label + badge key (`3/4`, `board`)
- **Command deck:** glass panel, brand header, live status chips, icon rows, pop-in, 3Ã—3 glyph trigger (replaces plain â˜°)
- Tools inject with icons; status bar refreshes while menu open

## 2026-07-12 â€” â˜° menu dedupe (v21)

- Flattened upper-right menu: Chat Â· Tools Â· Hide Â· Collapse Â· Apps Â· Appearance Â· Help
- **Permission** is one cycle control (Always â†’ Ask â†’ Plan), same as livebar
...
## 2026-07-12 â€” Keyboard clearance + rotate formatting (v20)
...
- **Rotate:** hard-reset body pin + CSS vars (`--vv-h`, `--feed-pad-bottom`, â€¦); multi-stage reflow through ~1.6s; detect width/height flip as orientation
...
## 2026-07-09 â€” Mobile keyboard + orientation reflow
...
- **Rotate** portraitâ†”landscape: multi-stage reflow (reset composer height, layout, stack pad, scroll)
- Permission chip/menu; always-approve skips plan approve UI
- Markdown lettered lists + safer path chips

## 2026-07-09 â€” Fix mixed history across sessions
...
## 2026-07-09 â€” Start server reliability + faster chat open
...
## 2026-07-09 â€” Fast history: last 100 + scroll-up load

- Open session loads only the **latest ~100** disk events (smaller read window)
- **Scroll up** (or tap â€œearlier messagesâ€) loads the previous page and prepends it
- API: `/api/session/history?limit=100&before=<byte>` + `meta.has_more` / `older_before`

## 2026-07-09 â€” Footer de-clutter + menu fonts + Amni-Delve

- â˜° menu uses UI `--font` / size (no fixed 12px mono)
- Removed above-composer clutter: no more â€œnot a git repoâ€, cost strip, or always-on When busy row
- Git + turns/ctx move into the **livebar** (only when useful); Interject/Queue/FYI only while busy
- **Amni-Delve** always in â˜° Â· Voice & wear; owner auto-unlock for Anthony/Amni paths

## 2026-07-09 â€” Fix new chat inheriting old user messages
...
## 2026-07-09 â€” Composer UX + XR detect + Watch v19

- **+** is attach-only; **Mic** sits beside the composer for Go voice
- Special functions moved to top-right **â˜°** (Todos, Terminal, Git, Export, â€¦)
- **Chat view** toggles in â˜°: hide/collapse thinking, tools, edits, reads, code
...
## 2026-07-09 â€” Message queue modes + Cancel v18

- **Interject / Queue / FYI** chips above composer (`When busy`)
- **Interject:** cancel current steps â†’ send now
- **Queue:** hold until a good pause (`turn_completed`), then send with guidance framing
- **FYI:** extra context only â€” never cancels; applied at next pause with â€œdonâ€™t stopâ€ framing
...
## 2026-07-09 â€” Voice Go + XR/AR + Grok TTS v17
...
- **Real Grok voice:** `POST /api/tts` â†’ `https://api.x.ai/v1/tts` (needs `XAI_API_KEY`); browser `speechSynthesis` fallback
- `GET /api/voice/status` reports TTS readiness; voice picker (eve, ara, leo, â€¦)

## 2026-07-09 â€” Promote archived â†’ live on message v16

- Messaging an **archived** session unarchives it (`POST {id, archived:false}`), marks it **live**/resident, switches scope Archived â†’ Active, and **auto-refreshes** the session list
...
## 2026-07-10 â€” Polish v15 (syntax, greyscale, tools, tour, repo)
...
## 2026-07-10 â€” Spoiler-safe Grok screenshots v14
...
- README shots retaken â€” no personal paths/chats, Grok dark default, phone without Alt

## 2026-07-10 â€” Menu stacking + code line numbers / syntax

- Header â˜° / orbit menus use `position:fixed` + high z-index so they paint above chat
...
## 2026-07-10 â€” Live CLIâ†’remote disk catch-up
...
## 2026-07-10 â€” Start Server + port claim + reconnect guard v8

- **Start Server** header + setup button â†’ `POST /api/stack/start` (kills stale :2419, spawns `grok agent --no-leader serve`, rebinds hub)
- **Boot:** UI claims free :2421 listeners; auto-starts agent if missing; `start.ps1` kills :2419/:2421 before bind
- **Reconnect:** max 5 tries, no infinite reconnecting #N loop; agent error no longer thrashing; after give-up â†’ hit Start Server
- Auto-connect: if `/health` agent down, Start Server first then connect

## 2026-07-10 â€” Live status + init/session reliability v7
...
## 2026-07-10 â€” Pin fix + feature tour v6

- **Pin bugfix:** local `import subprocess` inside `main_async` except-block made nested handlers fail with *cannot access free variable 'subprocess'â€¦* when aiohttp was already installed. Use module-level `subprocess` only.
...
## 2026-07-10 â€” Stop button + pin/start from anywhere

- UI header **Stop** â†’ `POST /api/stack/stop` (UI + agent serve only; never mass-kill grok)
- UI **Pin** â†’ `POST /api/stack/shortcut` creates Desktop + Start Menu shortcuts
...
## 2026-07-09 â€” Claude-gap pack v5.1

Closer to Claude Code day-to-day flow:

- **Stop turn** button (session/cancel) while agent is busy
- **Git strip** branch/sha/dirty + `/api/git/status|diff|log` + Î” working-tree diff
...
- **Open tool paths in IDE** (click ðŸ“„ locs)
...
## 2026-07-09 â€” UX fix: contrast, horizon loader, terminal lag v4.2

- Contrast: solid bubbles, readable `--tx`/`--soft`/`--mut`, softened Grok mono (no pure black/white)
- **Event horizon loader** lives in `#chatStage` (chat area above composer only â€” not header/fullscreen)
...
## 2026-07-09 â€” xAI product UX polish v4.1b
...
## 2026-07-09 â€” Claude-gap cockpit features v5.0
...
## 2026-07-09 â€” Full skills palette v4.3
...
- Advanced skills (design, execute-plan, review, â€¦) appear even if agent list is sparse
- Restart UI server required

## 2026-07-09 â€” Multi-client hub + history snap v4.2
...
## 2026-07-09 â€” Personas & directions v4.1

- **Persona flyout:** personalities (Concise, Unhinged, Programmer, Engineer, Manager, Clown, Warlord, â€¦)
...
## 2026-07-09 â€” Cockpit IDE + auto-stack + Grok Review v4.0

- **Electron auto-stack:** launch app â†’ agent + UI proxy (no daily terminal)
...
## 2026-07-09 â€” X-like labels + Ubuntu Yaru-ish v3.7
...
## 2026-07-09 â€” Tool payloads + rename skins v3.6
...
## 2026-07-09 â€” Code colors + collapsible rail + flyouts v3.5

- **Syntax colors** in fenced code: keywords/strings/numbers/comments/fns use theme tokens (`--acc`, `--ok`, `--you`, â€¦)
- **Collapsible sessions rail:** Â« Hide / â˜° / edge tab; pref `grok_remote_sidebar`
- **Top-right flyouts:** Theme, Skills, Task open frosted glass panels sliding from top-right (Esc closes)

## 2026-07-09 â€” Legacy themes v3.4
...
- Theme picker groups **Product** then **Legacy Â· retro**
- Win95 bevel chrome, Amiga/Win95 title bars, Matrix phosphor glow

## 2026-07-09 â€” Chat polish + attachments v3.3
...
- **File upload:** ðŸ“Ž attach / drag-drop / paste; images as ACP `image` blocks; text/code as resource + fenced body
- Limits: 8 files, 6MB each Â· hard-refresh clients (`?v=3`)

## 2026-07-09 â€” SpaceXAI beauty + session isolation v3.2
...
## 2026-07-09 â€” Sessions scroll + jump + archive v2

- **â†“ Bottom** always on in chat (not only when scrolled up); forced scroll + `scrollIntoView`
- **Independent scrolls:** body locked; session rail list vs chat feed scroll separately
- **Archive / Unarchive** per session (localStorage); scope chips Active Â· Live Â· Archived Â· All + search
- Layout: picker-head sticky controls, `.sess` flex scroll region

## 2026-07-09 â€” UX options v1

- **Auto-scroll:** toggle on setup/Theme UX chips; stick-to-bottom while reading live; jump FAB
- **Collapse:** thinking rows, fenced code, tool cards â€” defaults + per-block tap
